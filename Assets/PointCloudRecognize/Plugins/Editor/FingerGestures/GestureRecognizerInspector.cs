...
public abstract class GestureRecognizerInspector<T> : Editor where T:GestureRecognizer
{
    ...
    {
        Gesture1.RequiredFingerCount = Mathf.Clamp( Gesture1.RequiredFingerCount, 1, 50 );
    }
    ...
    {
        if( Gesture1.RequiredFingerCount > 1 && !Gesture1.SupportFingerClustering )
    ...
    protected virtual void OnSettingsUI() 
    {
        ...
        if( ShowRequiredFingerCount )
            Gesture1.RequiredFingerCount = EditorGUILayout.IntField( LABEL_RequiredFingerCount, Gesture1.RequiredFingerCount );
        else
            EditorGUILayout.IntField( LABEL_RequiredFingerCount, Gesture1.RequiredFingerCount );

        Gesture1.IsExclusive = EditorGUILayout.Toggle( LABEL_Exclusive, Gesture1.IsExclusive );

        GUI.enabled = Gesture1.SupportFingerClustering && !Gesture1.IsExclusive;

        if( GUI.enabled )
            Gesture1.MaxSimultaneousGestures = EditorGUILayout.IntField( LABEL_MaxSimultaneousGestures, Gesture1.MaxSimultaneousGestures );
        else
            EditorGUILayout.LabelField( LABEL_MaxSimultaneousGestures, NotAvailable );

        GUI.enabled = true;

        Gesture1.ResetMode = (GestureResetMode)EditorGUILayout.EnumPopup( LABEL_ResetMode, Gesture1.ResetMode );
    ...
    protected virtual void OnMessagingUI()
    {
        ...
        Gesture1.UseSendMessage = EditorGUILayout.Toggle( "Enable Message Broadcast", Gesture1.UseSendMessage );
        GUI.enabled = Gesture1.UseSendMessage;
        string eventName = string.IsNullOrEmpty( Gesture1.EventMessageName ) ? Gesture1.GetDefaultEventMessageName() : Gesture1.EventMessageName;
        Gesture1.EventMessageName = EditorGUILayout.TextField( LABEL_EventMessageName, eventName );
        Gesture1.EventMessageTarget = EditorGUILayout.ObjectField( LABEL_EventMessageTarget, Gesture1.EventMessageTarget, typeof( GameObject ), true ) as GameObject;
        Gesture1.SendMessageToSelection = (GestureRecognizer.SelectionType)EditorGUILayout.EnumPopup( LABEL_SendMessageToSelection, Gesture1.SendMessageToSelection );
        GUI.enabled = true;
    ...
    protected virtual void OnComponentsUI()
    {
        ...
        Gesture1.Raycaster = EditorGUILayout.ObjectField( LABEL_Raycaster, Gesture1.Raycaster, typeof( ScreenRaycaster ), true ) as ScreenRaycaster;
        Gesture1.Delegate = EditorGUILayout.ObjectField( LABEL_Delegate, Gesture1.Delegate, typeof( GestureRecognizerDelegate ), true ) as GestureRecognizerDelegate;

        GUI.enabled = Gesture1.SupportFingerClustering;
        if( Gesture1.SupportFingerClustering )
            Gesture1.ClusterManager = EditorGUILayout.ObjectField( LABEL_ClusterManager, Gesture1.ClusterManager, typeof( FingerClusterManager ), true ) as FingerClusterManager;
        else
            EditorGUILayout.ObjectField( LABEL_ClusterManager, Gesture1.ClusterManager, typeof( FingerClusterManager ), true );
        GUI.enabled = true;
    ...
    protected virtual void OnToolbar()
    {
        ...
        {
            string value = string.Format( "void {0}({1} gesture)", Gesture1.EventMessageName, Gesture1.GetGestureType().Name );
